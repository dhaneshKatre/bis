...
function retrieveBooks() {
	var container = document.getElementById("vb");
	var lang = ((document.getElementById("lang") || {}).value) || "";
	var genre = ((document.getElementById("genre") || {}).value) || "";
	...
	var fdb = firebase.database().ref('Books');
	fdb = fdb.child(lang).child(genre);
	fdb.on('value', function (snapshot) {
		//console.log('Snapshot '+ JSON.stringify(snapshot.val()));
		snapshot.forEach(function (item) {
			var author = item.child("Author");
			var content = item.child("Content");
			var descib = item.child("Describ");
			var rating = item.child("rating");
			var name = item.key;
			console.log(name);
			console.log(author.val());
			console.log(descib.val());
...
function bookL() {
	...
	var name = localStorage["bookName"];
	document.title = name;
	document.getElementById("bb").innerText = name;
	var lang = localStorage["lang"];
	var genre = localStorage["genre"];
	var fdb = firebase.database().ref('Books');
	img = fdb.child(lang).child(genre).child(name);
	img.on('value', function (snapshot) {
		var bookImg = document.getElementById("bookImg");
		var x = document.createElement("IMG");
		x.setAttribute("src", snapshot.val().url);
		x.setAttribute("width", "100px");
		x.setAttribute("height", "100px");
		bookImg.appendChild(x);
	});
	fdb = fdb.child(lang).child(genre).child(name).child("Content");
	fdb.on('value', function (snapshot) {
		snapshot.forEach(function (item) {
			var par = document.createElement("p");
			par.innerHTML = item.val();
